...
task Compile { 
	...
	$projects = ls "$solutionDir\source\**\*.csproj"
...
task Test { 
	...
	$projects = ls "$solutionDir\source\**\*.csproj" | ? { $_.Name -like "*Tests*" }
...
task Package -depends Compile { 
	...
	$projects = ls "$solutionDir\source\**\*.csproj" | ? { (-not ($_.Name -like "*Tests*")) -and (-not ($_.Name -like "*Sample*")) }
